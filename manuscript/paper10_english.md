...
Our main technical result (Theorem, §3) demonstrates that any bipartite quantum system with einselected pointer basis and Markovian environment satisfies A1–A5 via the Schwinger-Keldysh closed-time-path generating functional. The Feynman path integral emerges as the coherent limit (Corollary 1). Onsager-Machlup stochastic dynamics emerges as the classical thermal limit (Corollary 2). The CR-signal measured in large language model substrates (Pødenphant Lund 2026d) emerges as a discrete-time Keldysh observable with testable scaling properties (Corollary 3).
...
**The structural prediction.** Any system satisfying R1, R2, and R3 (jointly, in the sense above) will exhibit **non-trivial interior structure on its rate-parameter** — the performance function cannot be uniformly monotone, because the boundary regimes are qualitatively distinct dynamics from the interior. For substrates with monotone-survival ("Boltzmann-like") kernels — exponential or near-exponential decay of candidate viability under resource pressure — this interior structure takes the specific form of a performance function that vanishes (or near-vanishes) at the lower and upper bounds of the rate-parameter, with a single interior maximum. The argument for this Boltzmann-kernel-specific shape runs in three steps:
...
**Falsification criterion (revised, non-circular).** The framework asserts that every substrate passing the three independent operational tests above (R1 parallelism, R2 bounded resources, R3 irreversible commit, applied without reference to the rate-performance shape) exhibits *some* non-trivial interior structure on its rate-parameter — performance cannot be uniformly monotone across the entire rate range. A clear counter-example would be a substrate that passes the three independent operational tests and exhibits *strictly monotone* (no interior structure at all — no maxima, no minima, no inflection points, no branch-changes) performance on its rate-parameter. Non-inverted-U interior shapes — bimodal spikes, U-shape, multimodal patterns — are *not* counter-examples; they are kernel-specific outcomes consistent with the framework's structural prediction.

Because the operational criteria for R1+R2+R3 are stated in substrate-physics terms (interference, energy bounds, entropy production) without reference to the rate-performance shape, this falsification criterion is non-circular. Substrates that fail one or more operational criteria are *not* counter-examples because the failure can be diagnosed prior to examining their rate-performance behaviour. A genuine counter-example would have to be a substrate passing all three criteria and exhibiting *strictly* monotone rate-dependence with no interior structure of any kind. We have not constructed such a counter-example and conjecture that none exists for substrates with finite-dimensional kernel families, but this conjecture is empirically open.
...
### 4.3 Computational regime (discrete-time)
...
**Positioning relative to an adjacent framework:** Tarraga et al. 2026 (arXiv:2603.30031) presents a competing narrow-scope formalism for "cognitive friction" in LLM tool-agents (Triadic Cognitive Architecture, HJB optimal stopping). Paper 10's Corollary 3 shows how FT's substrate-universal formulation subsumes this as an LLM-specific special case of the broader race-dynamics.
