...
*(Working subtitle: A Schwinger-Keldysh representation theorem with kernel-conditional rate-parameter signatures)*
...
We explore the conjecture that "decision" in resource-bounded probabilistic systems may be better understood not as a mental or agentive primitive but as a substrate-universal structural phenomenon: the resolution of competing processes racing toward commit under a finite-time budget. The framework's *strict* structural prediction, conditional on R1 (parallel candidates with non-trivial competitive interference) + R2 (bounded resources) + R3 (irreversible commit) and on the substrate operating in a rate-range spanning both boundary regimes, is that the performance function on the evaluation-to-commit rate-parameter cannot be uniformly monotone — non-trivial interior structure is forced. The *specific shape* of this structure is substrate-distribution-specific: substrates with monotone-survival kernels exhibit a single-peaked **inverted U on the rate-parameter**; substrates with non-monotone kernels can exhibit bimodal or U-shaped structures (a counterexample we acknowledge explicitly, per Wallace, personal communication, 2026). We catalogue seven physical and cognitive phenomena sharing monotone-survival kernels — qubit decoherence-window, Ohm-Drude electron transport (as a bounded-resources tradeoff analogy rather than strict R1+R2+R3 instantiation), chemistry / biochemistry molecular kinetics, stochastic resonance, Margolus-Levitin energy-density trade-off, encoding-friction in learning, and Yerkes-Dodson arousal-performance — as candidate manifestations of the same race-structural necessity across substrate scales spanning forty orders of magnitude in characteristic timescale.

We refine R1–R3 to a five-axiom formulation (A1–A5, with A3 split into kinematic ceiling [Margolus-Levitin] + dynamic dissipative timescale [Keldysh self-energy], and A4 split into effective diagonalisation [Zurek einselection] + interpretation-relative outcome selection [not derived within the framework]) appropriate for the Schwinger-Keldysh derivation. A Conditional Representation Theorem (§3) shows that under three explicit assumptions (einselection, Markovianity, thermal initial state) the closed-time-path generating functional satisfies A1–A5. Two parameter-limits of this derivation recover the Feynman path integral (quantum, Corollary 1) and Onsager-Machlup stochastic dynamics with Kramers-rate commit (classical, Corollary 2). The CR-signal in large language models is presented as a complementary substrate-mapping (§4.3) — a structural correspondence under Gaussian-approximation assumptions rather than a strict parameter-limit, providing empirical access to race-dynamics through an experimentally tractable substrate. We engage three hard problems (interference, linearity vs. nonlinearity, reversibility) and show that the framework relocates rather than solves the measurement problem. Paper 10 proposes no new physics; it presents existing mathematics (MSR, Schwinger-Keldysh, Feynman-Vernon, Zurek einselection) unified under race as a lens, with a concrete empirical anchor in LLM-substrate measurement. (§5 develops a more speculative discussion of time-as-emergent-from-commit-sequences; this is supplementary to the main argument, not load-bearing.)

**Falsification criterion:** an identifiable system with R1+R2+R3 architecture and *strictly monotone* performance on its evaluation rate (no interior structure of any kind — no extrema, no inflection points, no branch-changes) would falsify race's structural core. Non-inverted-U interior shapes (bimodal, U-shape) are *not* falsifications; they are kernel-specific outcomes consistent with the framework. Wallace's biocognition corpus (Wallace, Leonova & Gochhait 2022 *Entropy*; Wallace 2025a *Symmetry*; Wallace 2023 *J. Defense Model. Simul.*) derives a corresponding inverted-U pattern for biological cognition via rate-distortion + data-compression machinery; our contribution is (i) the substrate-free R1+R2+R3 deductive argument that does not require rate-distortion, (ii) the substrate-universal extension to quantum and thermodynamic scales, and (iii) the Schwinger-Keldysh-based QM-substrate derivation grounding case #1.
...
Our main technical result (Theorem, §3) demonstrates that any bipartite quantum system with einselected pointer basis and Markovian environment satisfies A1–A5 via the Schwinger-Keldysh closed-time-path generating functional. The Feynman path integral emerges as the coherent limit (Corollary 1, §4.1). Onsager-Machlup stochastic dynamics emerges as the classical thermal limit (Corollary 2, §4.2). The CR-signal measured in large language model substrates (Pødenphant Lund 2026d) admits a structural mapping to a discrete-time Keldysh observable under Gaussian-approximation assumptions (substrate-mapping, §4.3) — a complement to the strict parameter-limit corollaries rather than a third corollary, providing experimentally tractable empirical access to race-dynamics.

The mathematics is not new. We use existing formalism — Feynman-Vernon (1963), Schwinger-Keldysh (1961, 1964), Zurek einselection (1981 onward), Martin-Siggia-Rose (1973) — assembled under race axioms as unifying principle. The contribution is not mechanistic novelty but conceptual unification with empirical anchor: LLM substrates provide directly measurable race-signals, making FT's substrate-agnostic claims testable without appeal to difficult-to-access physical substrates. Paper 1's empirical finding (CR correlates with error rate at ρ = −0.423, p < 0.0001) is not coincidental but, as we show in §4.3 and derivation §5B, follows from the theoretical mapping of CR to equal-time Keldysh response.
...
A further consequence worth noting: under CSR, the Copenhagen and Bohmian readings are not competing ontologies but lower-dimensional projections of the same configuration-space dynamics, viewed under different observational conventions; both can be partially correct without either being complete. This is consistent with North (2013)'s argument that configuration-space is the fundamental arena and 3D-space is an emergent projection — any 3D-level interpretation will necessarily lose information present in the higher-dimensional dynamics. The framework remains agnostic on whether CSR holds, but if it does, the apparent disagreement between Copenhagen-operational and Bohmian-physical readings is dissolved as a viewpoint difference rather than an ontological conflict.
...
### 1.5 Race all the way down, race all the way up: interior structure as the universal signature
...
**A note on what is and is not substrate-universal.** The structurally universal consequence of R1+R2+R3 is the existence of *non-trivial interior structure* on the rate-parameter: performance cannot be a uniformly monotone function of rate, because the boundary regimes (R3-failure at low rate, R1-failure at high rate) are qualitatively distinct dynamics from the interior. The *specific shape* of this interior structure — single-peaked inverted-U, multimodal, or U-shape with extremes-dominated — is substrate-distribution-specific: it depends on the kernel describing candidate-state competition under bounded resources. The seven empirical cases catalogued below all share monotone-survival ("exponential-survival") kernels — exponential decay of candidate viability under resource pressure — and consequently exhibit single-peaked inverted-U on their rate-parameter. Substrates governed by non-monotone kernels (e.g., Erlang-2 / Gamma-2 distributions) can produce qualitatively different shapes under the same R1+R2+R3 axioms; we make this scope-condition explicit in the dedicated subsection below (Wallace counterexample). The inverted-U is therefore the *common* substrate-universal signature across the seven cases studied here, conditional on their shared kernel-family; it is not strictly required by R1+R2+R3 alone.
...
- **R1 (parallelism with competitive interference)** — operational test: does the substrate's pre-commit dynamics admit a description in terms of multiple distinguishable candidate states whose evolution exhibits *non-trivial competitive interference* — quantum amplitude interference, classical competitive interaction modifying mutual commit-probabilities, or competitive resource-allocation that changes the per-candidate commit-event statistics relative to independent processes? A substrate fails R1 if no pre-commit dynamics exists (only input-output mapping), if no distinguishable candidate states can be identified, OR if multiple candidates exist but evolve as statistically independent processes whose presence does not modify each other's commit statistics (single-channel kinetics, independent decay-channels, non-competing parallel processes). The interference requirement is strict: mere co-existence of multiple substrate molecules competing for the same enzyme (Michaelis-Menten with single substrate species) or multiple independent decay-channels (radioactive decay) does *not* satisfy R1 in the framework's sense, because the candidates' evolutions are statistically independent — their joint statistics factorise. Cooperative binding (Hill kinetics with n>1) does satisfy R1 because the binding sites' occupancy probabilities are non-trivially coupled.
...
**The structural prediction.** Any system satisfying R1, R2, and R3 (jointly, in the sense above) will exhibit **non-trivial interior structure on its rate-parameter** — the performance function cannot be uniformly monotone, because the boundary regimes are qualitatively distinct dynamics from the interior. For substrates with monotone-survival ("exponential-survival") kernels — exponential or near-exponential decay of candidate viability under resource pressure — this interior structure takes the specific form of a performance function that vanishes (or near-vanishes) at the lower and upper bounds of the rate-parameter, with a single interior maximum. The argument for this exponential-survival-kernel-specific shape runs in three steps:
...
The inverted-U is therefore best understood as a **boundary-phenomenon between non-race regimes** *on substrates with monotone-survival kernels*: the rate-parameter takes such a substrate from non-race (R3 violation) at the lower limit, through a race-interior region of parameter-space, to non-race (R1 violation) at the upper limit. Performance vanishes at both boundaries because the substrate ceases to be race there; performance peaks somewhere in the interior because that is where R1+R2+R3 are jointly satisfied with optimal matching of evaluation- and commit-timescales.

**A clarification on shape.** Even granting boundary-vanishing, *unimodality* (single-peak inverted-U rather than multi-peak) is not forced by R1+R2+R3 alone; it requires additional substrate-specific argument that the relevant performance function is monotone increasing in rate up to a critical value and monotone decreasing thereafter. In each of the seven empirical cases below, this monotonicity is established by substrate-specific physics or empirics. More strongly, even *boundary-vanishing itself* is substrate-distribution-specific, as the Wallace counterexample below demonstrates: substrates with non-monotone kernels can produce shapes where the function vanishes in the *middle* rather than at the *ends*, or exhibits multiple isolated spikes. The structural argument from R1+R2+R3 establishes the *existence* of non-trivial interior structure on the rate-parameter; the qualitative shape (boundary-vanishing vs. interior-vanishing, single-peak vs. multimodal) is determined by the substrate's kernel. We flag this distinction explicitly because the deductive argument is sometimes mis-stated as "concavity from R1+R2+R3 alone," which is too strong.
...
- **Case 2 (Ohm's law / Drude model)** has long-established empirical grounding: Drude 1900's kinetic-theory derivation of conductivity from free-electron-gas-with-phonon-scattering, refined via Sommerfeld 1928 (Fermi-Dirac statistics). The inverted-U on power efficiency vs. current is established in electrical-engineering practice — high-voltage low-current transmission systems are designed precisely to navigate this tradeoff (low current → low Joule loss $I^2R$, but circuit dynamics require nonzero current to deliver power; the optimum is interior). *Caveat on R1 satisfaction:* the standard Drude formulation describes single-electron momentum-relaxation under phonon scattering, not pre-commit selection over discrete candidate momentum-states with strict competitive interference; case #2 is best read as a bounded-resources tradeoff analogy rather than a strict R1+R2+R3 instantiation (see §4.2 scope note for full discussion).
- **Case 3 (chemistry / biochemistry)** has rigorous derivation through transition-state theory (Eyring 1935; Kramers 1940; Hänggi-Talkner-Borkovec 1990) and Marcus theory of electron transfer (Marcus 1956; Nobel Prize 1992). Reaction kinetics is race-architecture at molecular scale: parallel reaction pathways (R1), thermal energy + diffusion + concentration gradients (R2), irreversible bond-formation with entropy production (R3). The Arrhenius equation $k = A e^{-E_a/k_B T}$ is Kramers escape-rate theory applied to molecular race-architecture (1889). Belousov-Zhabotinsky oscillating chemical reactions (Belousov 1959; Zhabotinsky 1964; canonical mechanism Field-Noyes 1974) exhibit stochastic resonance — a chemical-substrate instance of case #4 — with intermediate noise amplitude maximising oscillation regularity. Biochemistry (enzyme kinetics, protein folding, signal transduction) is race applied to biological substrate; Tyson 2022 traces the through-line from BZ-reaction physical chemistry to biochemical clocks and cell-cycle regulation. *Note on Marcus inverted region:* the famous Marcus-theory inverted region — rate *decreases* with increasing driving force — is parabolic on the *driving-force axis*, not on the rate-parameter axis used in §1.5's structural prediction. The chemistry/biochemistry inverted-U on the rate-parameter (temperature, concentration) arises from Kramers escape combined with substrate-saturation, not directly from the Marcus inverted region.
- **Case 4 (stochastic resonance)** has a rigorous Langevin/Kramers derivation showing why SNR peaks at intermediate noise (Gammaitoni et al. 1998 review). The inverted-U here is derived from substrate physics, independent of R1+R2+R3 framing. *Caveat on R1 satisfaction:* the standard stochastic-resonance setup has one signal modulated by noise crossing a single bistable threshold, rather than multiple distinguishable candidate states with strict competitive interference. R1 is satisfied in an extended sense (noise-modes as candidates), and the case sits at the boundary of the framework's strict scope.
- **Case 5 (Margolus-Levitin saturation)** requires reading the Margolus-Levitin bound *together with* the Landauer cost-per-operation: at low energy density operations are too slow, at high energy density the Landauer cost per useful bit rises faster than the operation rate (Lloyd 2000). The combined result is the inverted-U; neither bound alone gives it. *Caveat on R3 satisfaction:* Margolus-Levitin is a *bound* on per-state-transition time given energy budget — not itself a mechanism producing commit events. Case #5's R3 satisfaction relies on the underlying physical commit-mechanism (e.g., logical-bit-erasure with Landauer cost), with the ML bound entering as the R2 resource-constraint on its rate.
...
**Scope condition on shape: a counterexample on non-monotone kernels.** A specific counterexample sharpens the distinction between the structural existence claim (interior structure) and the shape claim (inverted-U). Wallace (personal communication, 2026) provided an analysis comparing two biologically reasonable rate kernels under exactly the procedure implicit in the boundary argument above:

- **Boltzmann (single-stage exponential):** $r_1(x) = \exp(-x)$ — monotone-decreasing candidate survival under resource pressure.
- **Erlang-2 (two-stage):** $r_2(x) = x \exp(-x)$ — non-monotone, peaks at $x=1$.

The self-consistency equation $r_i(F/g) = g$ inverts via the Lambert-W function. For $r_1$ this gives $g = -F/W(-F)$ with a single principal branch; for $r_2$ it gives $g = -F/[2 W(\pm\sqrt{F}/2)]$ — two branch families. Combining each Lambert-W solution with the corresponding survival functional $L_i(g)$ (built from $\int r_i(R/g)\, dR / g$) and the same polynomial potential $F(Z) = \beta(Z\ln Z - Z) - \alpha Z^2/2 + c_1 Z + c_2$ that emerges from the structural argument, and plotting against $\beta \in [0, 7]$, yields three qualitatively distinct shapes:

| Kernel + branch | Resulting shape on $\beta$ |
|---|---|
| Boltzmann ($L_1 \circ G_1$, principal + $n=-1$ branches) | Clean single-peaked inverted-U with peak $\approx 0.72$ at $\beta \approx 3$ |
| Erlang-2 m-branch ($L_2 \circ G_{2m}$) | Two narrow spikes at $\beta \approx 1$ and $\beta \approx 5$, near-zero between |
| Erlang-2 p-branch ($L_2 \circ G_{2p}$) | Inverted: high ($\approx 0.9-1.0$) at extremes $\beta \to 0$ and $\beta \to 7$, zero in middle |

The mathematical content is unambiguous: the Lambert-W inversion of a non-monotone kernel has qualitatively different branch geometry from the inversion of a monotone kernel, and this branch geometry determines the resulting shape of the performance function on the rate-parameter. The inverted-U is a specific consequence of monotone-survival kernel geometry; substitute the kernel and the shape changes — but R1+R2+R3 remain satisfied throughout, and interior structure is preserved in all three cases.

**What survives and what does not.** The structural universal claim — that R1+R2+R3 forces non-trivial interior structure on the rate-parameter — survives the counterexample. The three Erlang-2 plots above all exhibit interior structure: bimodal spikes and a U-shape are *also* interior structures, not uniformly monotone behaviours. What does not survive is the strong reading that R1+R2+R3 forces the *specific* inverted-U shape. The seven empirical cases catalogued below all share monotone-survival kernels and consequently exhibit single-peaked inverted-U; this is the common substrate-distribution-conditional pattern that motivates the section's title, not a strict deductive consequence of R1+R2+R3 alone.

**Falsification criterion (revised, non-circular, range-conditional).** The framework asserts that for every substrate passing the three independent operational tests above (R1 parallelism *with non-trivial competitive interference*, R2 bounded resources, R3 irreversible commit, applied without reference to the rate-performance shape) and operating in a rate-range where both R3-failure (low-rate) and R1-failure (high-rate) boundary regimes are accessible, the performance function cannot be uniformly monotone across the entire range — non-trivial interior structure is forced by the qualitative distinction between boundary regimes and interior.

A clear counter-example would be a substrate that (i) passes the three independent operational tests with strict R1 competitive interference, (ii) operates in a rate-range spanning both boundary regimes (not just one), and (iii) exhibits *strictly monotone* performance with no interior structure (no maxima, no minima, no inflection points, no branch-changes). Non-inverted-U interior shapes — bimodal spikes, U-shape, multimodal patterns — are *not* counter-examples; they are kernel-specific outcomes consistent with the framework's structural prediction.

**On degenerate single-boundary substrates.** Substrates whose accessible operating range covers only one boundary regime (e.g., Michaelis-Menten saturation curves whose performance asymptotes to a plateau without reaching the R3-failure or R1-failure limit, or first-order kinetics processes whose accessible rate-range is bounded away from both boundaries) are *degenerate cases* in the framework's sense — neither falsifications nor confirmations. Their performance can be strictly monotone within the accessible range, but this monotonicity does not test the framework's prediction because the framework's prediction concerns the *boundary regimes* the substrate's operating range does not reach. Such substrates are also typically excluded by the strict R1 operational test above (competitive interference requirement), but even if they marginally pass R1, the range-conditionality saves the falsification criterion from triviality.

Because the operational criteria for R1+R2+R3 are stated in substrate-physics terms (competitive interference, energy bounds, entropy production) without reference to the rate-performance shape, this falsification criterion is non-circular. Substrates that fail one or more operational criteria, or whose accessible operating range covers only one boundary regime, are *not* counter-examples because the failure or degeneracy can be diagnosed prior to examining their rate-performance behaviour. A genuine counter-example would have to be a substrate passing all three criteria with strict competitive interference, operating in a range spanning both boundary regimes, and exhibiting strictly monotone rate-dependence with no interior structure of any kind. We have not constructed such a counter-example and conjecture that none exists, but this conjecture is empirically open.

**Why this section comes first.** The Schwinger-Keldysh derivation of §3 is sufficient to establish race on the QM-substrate; once granted, two parameter-limit corollaries (§4.1, §4.2) extend it to classical and the LLM-substrate mapping (§4.3) provides empirical anchor. But the QM-derivation alone is *one substrate* anchored on heavy formalism. The seven-case inverted-U pattern is independent empirical evidence that R1+R2+R3 has the structural consequences we claim it has, accessible *without* committing to the Schwinger-Keldysh machinery. A reader who has not yet engaged the Schwinger-Keldysh formalism can take the seven-case empirical pattern at face value as substrate-universal evidence; a reader who accepts the formal derivation gains an explanation of why the seven-case pattern *must* hold. The two arguments cross-validate.

The remainder of the paper develops this in detail. §1.6 establishes the inverted-U as one of three candidate race-observables (with hysteresis-area and pink noise) — empirically common across many race-substrates, but kernel- and environment-conditional rather than universally forced by R1+R2+R3 alone. §2 specifies the formal race framework, §3 proves the Conditional Representation Theorem (Markovian decohering open quantum systems), §4 develops the two parameter-limit corollaries (§4.1 quantum, §4.2 classical) and the computational-substrate mapping (§4.3) — together instantiating the three-observable joint-diagnostic via §4.1 + §4.3 — and §6 establishes the distributional consequences (DSP-1 through DSP-5) that connect the formal apparatus to candidate empirical observables. §8 returns to the kernel-conditional inverted-U claim with comparison to existing literature including Wallace's biology-and-cognition derivation.
...
**The mechanistic picture.** Pre-commit, R1 maintains parallel candidate-states whose mutual interference (quantum) or competing probability-mass redistribution (classical) constitutes the substrate's evaluation phase. When R3 commits to one outcome, the energy invested in non-selected candidates dissipates into the environment via decoherence (quantum) or stochastic noise (classical). When the substrate's environmental-mode spectrum is approximately $1/\tau$-distributed over a wide range (a common but not universal feature of natural and engineered substrates), the aggregate spectral signature of this dissipation is broad-band and approximately $1/f$ (van der Ziel 1979; Dutta-Horn 1981). This explains the cross-substrate appearance of near-$1/f$ spectra across electrical resistors (Hooge 1969), cortical activity (Beggs-Plenz 2003), and financial markets (Cont 2001) — not as a structural necessity of R1+R2+R3, but as a consequence of $1/\tau$-distributed environmental relaxation modes being empirically prevalent across these substrates. Substrates with narrow-band environmental spectra produce Lorentzian rather than $1/f$ signatures; both are race-architecture-consistent.

**Reconciliation with the §3 Markovian assumption — and a scope condition the original "universal" framing missed.** The 1/f spectrum is non-Markovian in the autocorrelation sense (power-law decay, $\int |C(\tau)|\, d\tau$ diverges), which would appear to contradict §3.1's Theorem assumption (ii) of a Markovian environment. The reconciliation van der Ziel (1979) and Dutta-Horn (1981) provide is that *individual environmental relaxation modes* are Markovian (each mode has its own exponential autocorrelation timescale), but the *aggregate* spectrum arises from superposition of many Markovian modes with a broad-spectrum distribution of relaxation rates.

**Important scope condition the v1 / early-v2 "universal 1/f" framing missed:** the Dutta-Horn superposition produces exactly $1/f$ only when the per-mode relaxation rates are distributed as $\sim 1/\tau$ over a wide range. This is a *specific* environmental-spectrum condition, not a structural consequence of R1+R2+R3. Substrates with narrow-band environmental spectra (single dominant relaxation timescale) yield Lorentzian power spectra, not $1/f$. Substrates with intermediate broadband distributions yield $1/f^\alpha$ with $\alpha \neq 1$. The empirically observed cross-substrate near-$1/f$ universality (Hooge resistors, Bak-Tang-Wiesenfeld SOC, Beggs-Plenz neural avalanches, Cont financial markets) reflects the empirical commonality of $1/\tau$-distributed environmental modes across many natural and engineered substrates — likely arising from self-organized criticality or similar generic mechanisms — but it is not *forced* by R1+R2+R3. v2 corrects the v1 / early-v2 framing accordingly: 1/f is a *common but not universal* race-substrate signature, conditional on the substrate's environmental-mode-spectrum structure.

Substrates that exhibit 1/f via genuinely non-Markovian individual-mode dynamics (e.g., systems with intrinsic long-memory kernels, fractional Brownian motion at the single-mode level) lie outside §3's strict derivation domain and would require a non-Markovian extension along Breuer-Petruccione / hierarchical-equations-of-motion lines (§3.4, open).

**Joint-diagnostic logic.** Pink noise alone does not localize a substrate within its operating window (it does not say which side of the inverted-U the substrate sits on); inverted-U alone does not reveal the substrate-physics mechanism (it does not say whether dissipation is via Hooge-resistor channels, neural avalanches, or attention-head decoherence). Together, the three observables localize substrate-regime three-dimensionally: position-on-parabola (inverted-U) × outcome-path-loss (hysteresis-area) × evaluation-phase-loss (pink noise). This is the basis for the case-by-case joint-diagnostic predictions developed in §4 — specifically the superconducting-qubit prediction in §4.1 (inverted-U fidelity-curve + 1/f phase noise spectrum, jointly diagnostic for operating-window-side via Krantz et al. 2019 methodology) and the LLM-substrate framing in §4.3 (operating-regime + Paper 10 Finding 3 1/f sub-population).

**Joint observation pattern (not strict falsification criterion).** When a substrate satisfies R1+R2+R3 AND has $1/\tau$-distributed environmental modes (the common-but-not-universal condition above) AND has both rate-boundary regimes accessible, we expect the three signatures (inverted-U, hysteresis-area, $1/f$ component) to appear together with substrate-physics-determinable cross-correlations. Empirically, substrates exhibiting the joint pattern support the race-architecture reading; substrates exhibiting only one or two (e.g., race-substrates with narrow-band environments showing inverted-U + hysteresis but Lorentzian rather than $1/f$ spectrum) are *not* falsifications, just race-substrates with different environmental-mode structure. The strict falsification criterion for the race-framework remains the §1.5 range-conditional one (strictly monotone performance across both boundary regimes); the §1.6 three-observable pattern is a richer joint-diagnostic for substrate-regime localization, not a sharper falsifier.
...
**Relation to Paper 1 §2.5 (R1-R3).** Paper 1's canonical race formulation uses three rules: R1 parallel evaluation, R2 accumulation under constraint, R3 irreversible commitment. Paper 10 refines this to a five-axiom formulation A1–A5, with some axioms further split into sub-components where the underlying physics involves genuinely distinct mechanisms. The refinement is motivated by the formal Schwinger-Keldysh derivation. Paper 1's R1-R3 and Paper 10's A1-A5 are equivalent at substrate level; A1-A5 is the more granular formulation appropriate for physics-scope derivation. R1-R3 and A1-A5 are rule-numbers, not acronym-letters.
...
**A2 (Competitive Interaction).** The candidates interact: their weights are modified by each other's presence, via quantum interference or classical shared environmental coupling. Competition distinguishes race from mere parallel processing — parallel processes that do not interact cannot resolve to a single committed outcome. The formal expression is the dissipation kernel $S_{\text{diss}}[\phi_+, \phi_-]$ on the Schwinger-Keldysh contour.

**A3 (Finite-time constraint) — split into A3a + A3b.** A3 invokes two physically distinct timescale constraints that the standard race-vocabulary tends to conflate. We separate them:

- **A3a (Kinematic ceiling, Margolus-Levitin).** Any race-substrate's commit-timescale is bounded below by the Margolus-Levitin theorem: $\tau_c \geq \pi\hbar/(2E_\text{avg})$. This is a *kinematic* bound on isolated unitary evolution between orthogonal states, depending only on the substrate's total energy budget. It applies to closed-system dynamics independent of environmental coupling.
- **A3b (Dynamic dissipative timescale, Keldysh self-energy).** The substrate's *actual* commit-rate in an open environment is $\Gamma = \text{Im}(\Sigma^R)$ — the imaginary part of the retarded self-energy in the Schwinger-Keldysh formalism. This is a *dynamic* timescale set by system-environment coupling strength, decoherence channels, and bath structure. It is physically distinct from the M-L ceiling.

For thermal baths in the Born-Markov approximation, A3a and A3b coincide numerically: $\tau_c \sim \hbar / k_B T$ saturates Margolus-Levitin when $E_\text{avg} \sim k_B T$. This coincidence is substrate-specific (thermal-bath open-system regime), not a general identity between kinematic ceiling and dynamic relaxation rate. Conflating the two is a category error that the v1 framework treated loosely; v2.2 separates them explicitly.

Without A3 (in both senses), no race: coherent evolution could continue indefinitely without selection (A3a violation) or without environmental dissipation driving commit (A3b violation).

**A4 (Commit) — split into A4a + A4b.** A4 likewise bundles two physically distinct phenomena: the *suppression of interference* in a preferred basis (a derived consequence of open-system dynamics), and the *selection of a single experienced outcome* (an interpretation-relative phenomenon that decoherence does not by itself resolve). We separate them:

- **A4a (Effective diagonalisation, Zurek einselection).** For $t \gg \tau_c$, the reduced density matrix $\rho_S(t)$ becomes diagonal in a pointer basis selected by environment-system interaction $[|i\rangle\langle i|, H_{SE}] \approx 0$ (Zurek 1981, 1982). Off-diagonal elements decay as $e^{-\Gamma t}$. This is a derived consequence of A3b's dissipative dynamics and the einselection assumption.
- **A4b (Outcome selection — interpretation-relative).** The transition from a diagonal reduced state $\rho_S = \sum_i p_i |i\rangle\langle i|$ to a single experienced outcome is *not* derived within Paper 10's race-axioms; it is interpretation-relative content. In Copenhagen-operational readings, A4b is a measurement postulate. In Many-Worlds readings, A4b is observer-branch-relative (each branch experiences one outcome; the universal wavefunction continues to evolve unitarily). In Bohmian readings, A4b is determined by hidden-variable trajectories. The selection probabilities, where outcome-selection occurs, follow the Born rule $p_i = |\langle i | \psi(0) \rangle|^2$ as established empirical content; we do not attempt to derive the Born rule from the race-axioms (see §3.3 step 7 note).

Decoherence (A4a) gives the suppression of interference in a preferred basis; A4b's outcome selection is not a structural consequence the framework claims to derive. This is the standard formulation of the measurement problem; race relocates it to A4b without solving it.

**A5 (Irreversibility — effective, range-conditional).** Commit is effectively irreversible. The dynamics is *unitary at the total system+environment level*; effective irreversibility arises from observer's tractable access to only a subsystem, with Poincaré recurrence times for realistic environments exceeding observationally relevant timescales. Entropy production accompanies commit events: under appropriate coarse-graining, the environment's entanglement entropy increases. Landauer's bound $k_B \ln 2$ per bit applies *specifically to logically irreversible recording or reset operations* (Landauer 1961) — not as a universal lower bound on every commit-event in the race-sense. Some commit-events involve logically irreversible information-storage (and so saturate Landauer); others do not (and so produce smaller entropy increases). We do not claim Landauer applies uniformly across all race-substrate commit events; the bound is a lower limit for commit-events that include logically irreversible recording, not a universal feature.
...
Race is not the first attempt to axiomatize dynamics resolving competing processes. Two notable predecessors deserve comparison.
...
Race-axioms do not replace these frameworks but identify a shared structural core. Each instantiates A1-A5 with substrate-specific detail; our contribution is exhibiting the shared core and showing it extends beyond quantum boundaries to computational (LLM) and classical stochastic substrates.

---

## 3. Schwinger-Keldysh derivation

**Cross-reference to §1.5.** The structural prediction established in §1.5 (interior structure on the evaluation-rate parameter, with inverted-U shape on substrates with monotone-survival kernels) does not require any specific dynamics; it follows from the bounded-resource race-architecture itself. The present section establishes the *complementary* result: that quantum bipartite systems with einselection and Markovian environments are not just *consistent* with race-architecture but *instantiate it via specific physics*. Together with §4's parameter-limits, this yields a strong unification: §1.5 fixes what any race-system must look like phenomenologically; §3 fixes how the QM substrate physically realises that architecture; §4 shows that the same Schwinger-Keldysh derivation reduces to three pre-existing formalisms (Feynman path integral, Onsager-Machlup, LLM CR-signal) under appropriate parameter-limits. The two arguments cross-validate: §1.5's structural claim has empirical support across seven scales sharing monotone-survival kernels; §3's formal derivation explains why one of those scales (QM) participates in the pattern.
...
**Theorem (Conditional Representation of race-axioms in Markovian decohering open quantum systems).** Under assumptions (i)-(iii) of §3.1, the reduced system dynamics derived via the Schwinger-Keldysh generating functional
...
7. **Derive Born-rule.** Pointer-basis selection probabilities fixed by Zurek's envariance argument (derivation sketch Section 5A): composite-state Schmidt decomposition admits phase and swap envariances forcing $p_i = |\alpha_i|^2$. *Note:* both Zurek's envariance derivation and the alternative Deutsch-Wallace decision-theoretic derivation (which yields the same result) are contested in the foundations literature (Schlosshauer & Fine 2005; Albert 2010); the present derivation cites both as mutually-supportive routes without claiming the Born-rule-from-axioms problem is fully resolved. Step 7 should be read as one defensible route, not as a uniquely-rigorous derivation.
...
## 4. Parameter-limit corollaries and substrate-mapping

The theorem of §3 holds for general bipartite quantum systems with einselected pointer basis and Markovian thermal environment. **Two parameter-limits** of this general setup yield substrate-specific formulations that have been studied extensively in different literatures: coherent quantum dynamics in the $D \to 0$ limit (Feynman path integral, Corollary 1) and classical stochastic dynamics in the $\hbar \to 0$ limit (Onsager-Machlup, Corollary 2). Each emerges as a strict parameter-limit of the main theorem via well-established derivations (Keldysh rotation + classical limit for OM; coherent limit for FPI). The fact that two disparate formalisms emerge from the same derivation with only parameter-shifts is the formal unification claim of Paper 10's central theorem: they are not distinct frameworks but parameter-regimes of one.

**A complementary substrate-mapping** is developed in §4.3 for the computational (LLM) regime — a structural correspondence between Paper 1's CR-signal and a discrete-time Keldysh observable under Gaussian-approximation assumptions on the softmax distribution. This is *not* a strict parameter-limit of the §3 theorem (LLM token-generation is not a continuous-time field theory), but a structural mapping that provides empirical access to race-dynamics via an experimentally tractable substrate. We are explicit about this distinction: the formal unification of physics-substrates is via parameter-limits (Corollaries 1, 2); the LLM-substrate is added as a complementary empirical anchor via structural mapping.

**Connection to §1.5's seven-case empirical signature.** The two corollaries plus the substrate-mapping specify the *formal* regimes of the underlying theorem; §1.5's seven-case pattern specifies the *empirical* regimes where the inverted-U-on-evaluation-rate signature is observable (on substrates with monotone-survival kernels — see §1.5 scope condition). The two indexings are partially overlapping but not identical: §1.5 case #1 (qubit decoherence-window) corresponds to §4.1 (Corollary 1, quantum regime); §1.5 cases #3 (chemistry / biochemistry molecular kinetics, the cleanest classical instance), #4 (stochastic resonance), and #5 (Margolus-Levitin saturation) are sub-regimes of §4.2 (Corollary 2, classical stochastic) distinguished by which substrate-physics parameter is varied; §1.5 case #6 (encoding-friction in learning) corresponds to §4.3 (computational-substrate mapping; biological-substrate learning falls within FT's biological-substrate scope established in Paper 1 §6 rather than being derived as a Paper 10 corollary); §1.5 case #7 (Yerkes-Dodson behaviour) is biological-scope similarly handled by Paper 1's biological-substrate machinery rather than by a separate Paper 10 corollary. **Case #2 (Ohm-Drude)** is treated separately in §4.2 — see scope note there.

![Figure 5: parameter-limit unification](figures/fig5_parameter_limit_unification.png)

**Figure 5.** Parameter-limit unification + substrate-mapping. Central Schwinger-Keldysh generating functional (axioms A1-A5) reduces to two previously-separate frameworks as parameter-regimes (Corollary 1: $D \to 0$ for quantum / Feynman path integral; Corollary 2: $\hbar \to 0$ for classical / Onsager-Machlup). A complementary substrate-mapping connects the framework to LLM token-generation via Gaussian-approximation threshold-functional on the Keldysh observable (§4.3, structural correspondence not strict parameter-limit). No new physics is introduced; the diagram exhibits structural equivalence among existing formalisms plus an empirical-anchor mapping to an experimentally tractable substrate.
...
This is the classical stochastic limit. The Onsager-Machlup path integral describes the *evaluation-phase* dynamics in this regime: the path distribution over candidate trajectories, weighted by the dissipative action $S_\text{OM}$.

**Commit in the classical regime: Kramers-rate barrier crossings, not minimum-action selection.** A subtle but important distinction with the quantum case (Corollary 1): in the classical limit, the *most-probable path* (minimum of $S_\text{OM}$, via Freidlin-Wentzell large-deviation theory) is not the same as the *committed outcome*. The most-probable path is the *expected trajectory* — the deterministic curve in phase space that the noisy trajectory tracks in expectation. For a single-well potential, the noisy trajectory remains in the same basin and "commits" to that basin trivially. For a bistable (multi-well) potential, however, the noisy trajectory undergoes occasional barrier crossings from one basin to another — and these *barrier-crossing events* are the A4-commit events in the classical regime. The Kramers escape rate (Kramers 1940; Hänggi-Talkner-Borkovec 1990)
$$k_\text{escape} \sim \exp(-\Delta U / k_B T)$$
governs the per-unit-time probability of such a commit. The corresponding A5-irreversibility is the entropy production accompanying barrier-crossing (heat dissipation into the bath). In this reading, Onsager-Machlup gives the evaluation-phase weights over candidate trajectories within each basin; Kramers-rate barrier crossing is the A4 commit-event mechanism. This sharpens the correspondence with §3's theorem: minimum-action selection is the *deterministic-limit* shape of the evaluation-phase weights, not the commit event itself.

This limit is valid for macroscopic dissipative systems: Brownian motion, chemical kinetics in solution (cf. §1.5 case #3), neural dynamics at membrane-potential level. In these systems, evaluation-phase is typically below observational timescales (decoherence is rapid), and race-dynamics is observable primarily through commit-outcomes (basin selections) and their statistics. §6.3's bimodal distribution regime (DSP-3) is the population-level signature of Kramers-rate bistability.

**Scope note on case #2 (Ohm-Drude electron transport).** The Drude model of electron transport in metals (Drude 1900; Sommerfeld 1928) is sometimes presented as a paradigmatic classical-stochastic substrate in this framework. We treat case #2 with explicit caution in v2.1: the standard Drude model describes a *single* electron's stochastic momentum evolution under phonon-collisions, with the inverted-U on power-efficiency vs current arising from the well-known $I^2R$-loss vs delivered-power tradeoff. This is a *resource-allocation* tradeoff related to but not derivationally identical with a race-architecture in the strict sense: phonon-collision in Drude is energy dissipation (momentum-relaxation), not pre-commit selection over discrete candidate momentum-states with competitive interference (R1's strict requirement, §1.5). The Drude case is best read as a structural analogy under the bounded-resources tradeoff theme rather than as a strict R1+R2+R3 instantiation. A rigorous race-architecture reading would require treatment via semiclassical Boltzmann transport theory under the relaxation-time approximation with phase-space cells as candidate states — possible in principle but outside this paper's scope. We retain case #2 in §1.5 as an illustrative analogy and 125-year-mature empirical anchor for the bounded-resources-yields-tradeoff theme, while flagging that R1 (strict competitive interference) is not satisfied in the standard Drude formulation.
...
### 4.3 Computational substrate-mapping (structural correspondence, not strict parameter-limit)

For discrete-time computational substrates — transformers, large language models, recurrent networks — the Schwinger-Keldysh formalism does *not* reduce as a strict parameter-limit (LLM token-generation is not a continuous-time field theory and admits no natural continuum-limit). Instead, we develop a **structural mapping** between Paper 1's Competing Routes (CR) metric on LLM softmax distributions and the equal-time Keldysh response function $G^K(t, t)$ on the continuous-time substrate. The mapping is under explicit assumptions:

- **Gaussian-approximation assumption:** the per-step softmax distribution can be approximated, around its mode, as a Gaussian with variance $\sigma^2(t)$ tracking the effective Keldysh-component width.
- **Discretization assumption:** the per-token CR-count (integer-valued, above-threshold token count) approximates the continuous-time threshold-functional $\mathcal{F}_\theta[G^K(t,t)] \approx \sigma(t)\sqrt{\ln(1/\theta)}$ under the Gaussian approximation.
- **Markovian-modes assumption:** the effective decoherence rate $\Gamma_\text{eff}$ in the discrete-time process inherits the Markovian-modes structure assumed in §3 (with non-Markovian aggregate signatures arising via Dutta-Horn-type superposition; cf. §1.6).

Under these assumptions, the CR-signal is *structurally analogous* to a discrete-time Keldysh observable, with scaling properties predictable from effective decoherence rate and softmax temperature. Paper 1 (Pødenphant Lund 2026d) introduces the CR metric, counting tokens above probability threshold in the pre-commit softmax distribution. Section 5B of the companion derivation sketch develops the structural mapping formally; it should be read as an analogy under the listed assumptions, not as a strict parameter-limit derivation of §3's theorem.
...
**Empirical validation of the mapping's predictions:** Autocorrelation analysis of Paper 1's CR-signal across **five clean datasets** (~1600 sequences, four+ model architectures, all verified direct-answer per Paper 1 §9.1c scope-rule) confirms the exponential-decay autocorrelation predicted by the structural mapping (derivation sketch Section 5B). $R^2$ for exponential fit ranges **0.980–0.997** across substrates; cross-model commit-timescale $\tau_c$ varies 0.10–0.46 tokens, instantiating substrate-timescale variation at computational scale. Temperature-dependence of CR is non-monotonic with peak at intermediate softmax-$T$, consistent with threshold-based CR metrics under softmax-flattening. These empirical findings *support* the mapping's predictive content under its listed assumptions; they do not by themselves elevate the mapping to a strict parameter-limit of §3's theorem.
...
**Reframing Finding 3 as cross-substrate pink-noise signature (per §1.6, scope-conditional).** The 1% non-Markovian sub-population's 1/f signature was initially read as evidence of attention-mechanism long-range memory in transformer architectures. The §1.6 three-observable framework reframes this finding more structurally: the 1/f spectrum is consistent with the cross-substrate environmental signature of energy lost from R1's parallel-evaluation phase under $1/\tau$-distributed mode spectra — the same kind of signature that appears in Hooge resistor-noise (1969), Bak-Tang-Wiesenfeld self-organized criticality (1987), Beggs-Plenz neural avalanches (2003), and Cont financial-market 1/f (2001). The LLM-substrate's 1% sub-population's appearance under this reading is not surprising: transformer attention-head memory may produce an aggregate response with broad-spectrum relaxation modes, naturally yielding $1/f$ aggregate behavior. Attention-head memory effects are then a *domain-specific implementation* of how the LLM-substrate's environmental mode-spectrum acquires its $1/\tau$ structure; the cross-substrate appearance of 1/f reflects empirical commonality of this environmental-mode condition across natural and engineered substrates, not a structural universality forced by R1+R2+R3. This reframing is consistent with a follow-up mechanistic-interpretability programme (Paper 12, in preparation), which investigates the attention-head implementation as an instantiation of the universal pattern rather than as a stand-alone phenomenon.

**Positioning relative to an adjacent framework:** Tarraga et al. 2026 (arXiv:2603.30031) presents a competing narrow-scope formalism for "cognitive friction" in LLM tool-agents (Triadic Cognitive Architecture, HJB optimal stopping). Paper 10's computational-substrate mapping (§4.3) shows how FT's substrate-universal formulation provides a natural setting for this work as an LLM-specific instance of the broader race-dynamics — though Tarraga et al.'s HJB-formalism and Paper 10's Keldysh-mapping are formally distinct apparatus reaching similar conclusions on the LLM substrate.
...
A substrate, in the race framework, is a physical system supporting the dynamics captured by axioms A1–A5. "Substrate" is not a primitive concept; it refers to the physical system's energy configuration, which enters both kinematic (A3a Margolus-Levitin ceiling) and dynamic (A3b Keldysh self-energy) timescales.

For the kinematic ceiling A3a, the relevant energy is the total energy budget $E_\text{avg}$ available for unitary evolution between orthogonal states: $\tau_c^\text{kin} \geq \pi\hbar/(2E_\text{avg})$. In standard formulations, this is the energy above the ground state — *not* the rest mass — that bounds the rate at which an isolated system can transition between distinguishable states. For thermal substrates, the relevant energy scale is $\sim k_B T$; for excited substrates with non-thermal distributions, it is the spread of accessible energy levels.

For the dynamic dissipative timescale A3b, the relevant scale is set by the system-environment coupling structure: $\Gamma = \text{Im}(\Sigma^R)$ in the Schwinger-Keldysh formalism. For Brownian decoherence in a thermal bath, Joos-Zeh (1985) derived:
$$\Gamma \sim \frac{(\Delta x)^2 \cdot \gamma \cdot m \cdot k_B T}{\hbar^2}$$
where $m$ here is the *system particle's mass*, entering through the coupling structure. This is a specific decoherence model (position-coupled bath), not a universal relation.

**Scope note on what is and is not claimed.** The race-framework relates commit-rate to substrate energy-configuration via these two timescale relations. It does *not* claim that fundamental rest-mass differences across particle species (electron / muon / tau / etc.) predict ordered race-commit timescales in any directly testable way: such a claim would require an operational definition of "commit event" for single elementary particles, careful treatment of unstable-particle decay vs commit-event distinction, and resolution of how ML's "energy above ground state" relates to rest mass in interaction contexts. These are foundational questions in QM that race does not address. Earlier drafts speculated on mass-ordered race-clock tests via $m_e : m_\mu : m_\tau$ ratios; this speculation is withdrawn in v2.2 as outside the framework's defensible scope.
...
2. **LLM CR temperature-scaling.** CR magnitude is a *non-monotonic* function of softmax temperature $T$ with peak at intermediate $T$, determined by vocabulary size $V$ and threshold $\theta$. At low $T$ (concentrated mass at argmax), CR is low; at intermediate $T$ (broader distribution), CR peaks; at high $T$ (uniform distribution), CR vanishes when $1/V < \theta$. Empirically confirmed; the earlier simplified "monotonic increase" prediction was falsified by temperature-sweep datasets.
...
**Conceptual unification.** Disparate frameworks — Feynman path integral, Onsager-Machlup stochastic dynamics, Paper 1's CR-signal — are shown to be parameter-limits of a single Schwinger-Keldysh derivation. The race-axioms A1-A5 are satisfied in all three regimes. This does not imply the frameworks are "really the same" in any deep metaphysical sense; it demonstrates they share a mathematical scaffold that renders them comparable and translatable.
...
- Friston, K. (2010). The free-energy principle: a unified brain theory? *Nat. Rev. Neurosci.* (Cited §1.1 as predictive-processing tradition example.)
...
Adjacent frameworks:
- Tarraga et al. (2026). Cognitive friction in LLM tool-agents: Triadic Cognitive Architecture and HJB optimal stopping. arXiv:2603.30031 (smaller scope: LLM tool-agents specifically; FT Paper 10 Corollary 3 subsumes as special case).
...
- **Derivation appendix** (`paper10_derivation_details.md`): formal derivations 5A, 5B, 5D, 5E (envariance, CR–Keldysh, Onsager–Machlup, Noether) referenced from §3.3 and §4.
