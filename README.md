...
1. **Theoretical (kernel-conditional rate-parameter structure, §1.5):** Systems satisfying R1+R2+R3 (parallel candidates with non-trivial competitive interference + bounded resources + irreversible commit), operating in a rate-range spanning both boundary regimes, exhibit non-trivial interior structure on their evaluation-to-commit rate-parameter — performance cannot be uniformly monotone. The specific shape (single-peaked inverted-U, bimodal, U-shape) is substrate-distribution-specific: monotone-survival kernels yield inverted-U; non-monotone kernels yield other shapes per the Wallace counterexample (personal communication 2026; Lambert-W branch geometry on Erlang-2 kernel). Seven empirical cases sharing monotone-survival kernels are catalogued: qubit decoherence-window (Preskill 2018; Krantz et al. 2019), Ohm-Drude electron transport (bounded-resources analogy, not strict R1+R2+R3 instantiation), chemistry/biochemistry molecular kinetics (Arrhenius/Kramers/Marcus), stochastic resonance (boundary case), Margolus-Levitin saturation, encoding-friction in learning (Pødenphant Lund 2026b), and Yerkes-Dodson behavioural arousal. Wallace's biocognition rate-distortion programme (Wallace, Leonova & Gochhait 2022; Wallace 2025a, 2023) is the closest concurrent precedent in biology-and-cognition scope; FT extends to quantum + thermodynamic scales and provides a substrate-free deductive argument that does not require rate-distortion machinery.

2. **Theoretical (Schwinger-Keldysh, §3):** A Conditional Representation Theorem (§3.2) shows that under three assumptions (einselection, Markovianity, thermal initial state) the Schwinger-Keldysh closed-time-path generating functional satisfies race-axioms A1–A5. Two parameter-limit corollaries recover the Feynman path integral (quantum, Corollary 1) and Onsager-Machlup with Kramers-rate commit (classical, Corollary 2). A complementary substrate-mapping (§4.3) connects to Paper 1's CR-signal under Gaussian-approximation assumptions — structural correspondence, not strict parameter-limit. (Earlier drafts included an FEP-as-Corollary-3 derivation; this was removed in v2 after Wallace flagged it as scope-overreach.)
